...
void test_writes(uint64_t argument_1, uint64_t argument_2) {
    ...
    for (int i = 0; i < NUM_SAMPLES; i++) {
        testSmart::FPR_test_with_file<inc9>(filename, true, num_samples, read_size, max_capacity, false);
    }
    ...
    for (int i = 0; i < NUM_SAMPLES; i++) {
        testSmart::FPR_test_with_file<BCF>(filename, true, bcf_num_samples, bcf_read_size, max_capacity, false);
    }
