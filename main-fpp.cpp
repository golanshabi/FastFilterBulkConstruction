...
void run_FPR_test(const std::string &name, std::vector<u64> *v_add, const std::vector<u64> *v_find, const std::string &path, size_t filter_max_capacity, bool is_hashable = false, bool create_file = false, bool sorted = false, SortType sort_type = SortType::RadixAVX2) {
    for (int i = 0; i < NUM_SAMPLES; i++) {
        testSmart::FPR_test<Filter>(name, v_add, v_find, path, is_hashable, create_file, sorted, sort_type, filter_max_capacity);
    }
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <exponent> [radix_bits]" << std::endl;
        return 1;
    }

    if (argc >= 3) {
        setenv("RADIX_BITS", argv[2], 1);
    }
...
void write_fpp_to_file(size_t fp_max_capacity) {
    ...
    if (run_bcf)
        run_FPR_test<PQF::PQF_8_53>("PQF_8_53_bulk", &bcf_v_add, &bcf_v_find, path, fp_max_capacity, true, false, true);
    run_FPR_test<PQF::PQF_8_53>("PQF_8_53", &bcf_v_add, &bcf_v_find, path, fp_max_capacity);
    ...
    run_FPR_test<inc9>("IPS2RA", &fp_v_add, &fp_v_find, path, fp_max_capacity, true, false, true, SortType::IPS2RA);
    ...
    // run_FPR_test<inc9>("RadixBuiltSlowlySIMD", &fp_v_add, &fp_v_find, path, fp_max_capacity, true, false, true, SortType::RadixBuiltSlowly);
    #endif
}
