...
namespace min_pd {
    ...
    constexpr size_t MAX_WITH_SPARE = MAX_CAP0 + 7;
    ...
    inline bool is_pd_full(const uint64_t header) {
        bool res = header & (1ULL << 55);
        assert(res == (get_cap(&pd) == MAX_CAP0));
        return res;
    }
    ...
    inline size_t get_spec_quot_cap_from_header(size_t quot, uint64_t &header) {
        const u64 clean_h = (header >> 6ul) & ((1ULL << 50ul) - 1);
        if (quot == 0) {
    ...
    inline size_t get_spec_quot_cap(size_t quot, const __m256i *pd) {
        assert(quot < QUOTS);
        return get_spec_quot_cap_from_header(quot, (((uint64_t *) pd)[0]));
    }
    ...
    inline void body_add_case0_avx(size_t body_index, uint8_t rem, __m256i &pd) {
        constexpr unsigned kBytes2copy = 7;
        const __m256i shifted_pd = shift_right(pd);
        const uint64_t cur_cur_header = reinterpret_cast<const u64 *>(&pd)[0];
        ((uint8_t *) &pd)[kBytes2copy + body_index] = rem;
        const uint64_t mask = _bzhi_u64(-1, kBytes2copy + body_index + 1);
        pd = _mm256_mask_blend_epi8(mask, shifted_pd, pd);
    }
    ...
    inline size_t sort_last_quot_with_header(__m256i *pd, u64 clean_h) {
        // assert(is_pd_full(pd));
        // const u64 clean_h = get_clean_header(pd);
        const size_t last_quot = select64(~clean_h, MAX_CAP0 - 1) - (MAX_CAP0 - 1);
    ...
    inline size_t sort_last_quot(__m256i *pd) {
        const u64 clean_h = get_clean_header(pd);
        return sort_last_quot_with_header(pd, clean_h);
    }
