...
namespace PQF {
    ...
    class PartitionQuotientFilter {
        ...
        private:
            ...
            // A concurrent relocation is planned using stable bucket/crumb identities, never
            // slot indices: removing or inserting any entry can shift every later slot.
            struct BulkRelocationMove {
                std::size_t sourceBucketIndex;
                std::size_t destinationBucketIndex;
                std::size_t crumb;
            };

            struct BulkRelocationPlan {
                std::size_t count = 0;
                BulkRelocationMove moves[BulkRelocationDepth];
            };
            ...
            inline BulkBucketScan scanFullBulkBucket(
                    std::size_t backyardBucketIndex) {
                assert(backyard[backyardBucketIndex].full());
                ...
                for (std::size_t keyIndex = 0;
                     keyIndex < BackyardBucketCapacity; ++keyIndex) {
            ...
            inline bool bulkBucketWasVisited(
                    std::size_t backyardBucketIndex,
                    const std::size_t (&visited)[BulkRelocationDepth],
                    std::size_t visitedCount) const {
                for (std::size_t i = 0; i < visitedCount; ++i) {
                    if (visited[i] == backyardBucketIndex) {
                        return true;
                    }
                }
                return false;
            ...
            // Build an optimistic relocation path while synchronizing every bucket read. Locks
            // are deliberately taken one at a time here: the complete bucket set is not known
            // until the path has been found. The caller later reacquires the whole set in index
            // order and revalidates it before changing anything.
            template<typename BucketLocks>
            inline bool planBulkRelocation(
                    std::size_t backyardBucketIndex,
                    std::size_t depth,
                    std::size_t (&visited)[BulkRelocationDepth],
                    std::size_t visitedCount,
                    BucketLocks& locks,
                    BulkRelocationPlan& plan) {
#ifdef CUCKOO_HASH
                (void) backyardBucketIndex;
                (void) depth;
                (void) visited;
                (void) visitedCount;
                (void) locks;
                ...
                return false;
#else
                locks.Lock(backyardBucketIndex);
                if (!backyard[backyardBucketIndex].full()) {
                    locks.Unlock(backyardBucketIndex);
                    plan.count = 0;
                    return true;
                }
                const BulkBucketScan scan = scanFullBulkBucket(backyardBucketIndex);
                locks.Unlock(backyardBucketIndex);

                const std::uint32_t candidates =
                        scan.crumbsPresent & BulkUsableCrumbMask;

                // Preserve the serial search's preference for a one-hop vacancy before
                // recursively considering longer relocation chains.
                for (std::uint32_t crumbs = candidates;
                     crumbs != 0; crumbs &= crumbs - 1) {
                    const std::size_t crumb = __builtin_ctz(crumbs);
                    const std::size_t destinationBucketIndex =
                            bulkAlternativeBucket(backyardBucketIndex, scan, crumb);
                    if (destinationBucketIndex == backyardBucketIndex ||
                        destinationBucketIndex >= backyard.size() ||
                        bulkBucketWasVisited(destinationBucketIndex, visited,
                                             visitedCount)) {
                        continue;
                    }

                    locks.Lock(destinationBucketIndex);
                    const bool hasRoom = !backyard[destinationBucketIndex].full();
                    locks.Unlock(destinationBucketIndex);
                    if (hasRoom) {
                        plan.count = 1;
                        plan.moves[0] = BulkRelocationMove{
                                backyardBucketIndex, destinationBucketIndex, crumb};
                        return true;
                    }
                }
                ...
                for (std::uint32_t crumbs = candidates;
                    ...
                    BulkRelocationPlan suffix;
                    if (planBulkRelocation(destinationBucketIndex, depth - 1,
                                           visited, visitedCount, locks, suffix)) {
                        plan.count = suffix.count + 1;
                        plan.moves[0] = BulkRelocationMove{
                                backyardBucketIndex, destinationBucketIndex, crumb};
                        for (std::size_t i = 0; i < suffix.count; ++i) {
                            plan.moves[i + 1] = suffix.moves[i];
                        }
                        return true;
                    }
                }
            ...
            // Requires all target and plan buckets to be locked. A plan may have gone stale
            // between discovery and ordered acquisition, so validate every crumb and find the
            // earliest current vacancy before applying the chain from its tail to its root.
            template<typename BulkTarget>
            inline bool tryBulkRelocationAndInsert(
                    const BulkTarget& target,
                    const BulkRelocationPlan& plan) {
                if (!backyard[target.firstBackyardQR.bucketIndex].full() ||
                    !backyard[target.secondBackyardQR.bucketIndex].full()) {
                    return insertOverflow(target.overflow, target.firstBackyardQR,
                                          target.secondBackyardQR);
                }
                if (plan.count == 0) {
                    return false;
                }

                std::size_t movesToApply = 0;
                for (std::size_t i = 0; i < plan.count; ++i) {
                    if (!backyard[plan.moves[i].destinationBucketIndex].full()) {
                        movesToApply = i + 1;
                        break;
                    }
                }
                ...
                BulkBucketScan scans[BulkRelocationDepth];
                for (std::size_t i = 0; i < movesToApply; ++i) {
                    const BulkRelocationMove& move = plan.moves[i];
                    if (!backyard[move.sourceBucketIndex].full()) {
                        return false;
                    }
                    if (i != 0 &&
                        plan.moves[i - 1].destinationBucketIndex !=
                                move.sourceBucketIndex) {
                        return false;
                    }

                    scans[i] = scanFullBulkBucket(move.sourceBucketIndex);
                    const std::uint32_t crumbBit = 1u << move.crumb;
                    if ((scans[i].crumbsPresent & crumbBit & BulkUsableCrumbMask) == 0 ||
                        bulkAlternativeBucket(move.sourceBucketIndex, scans[i], move.crumb) !=
                                move.destinationBucketIndex) {
                        return false;
                    }
                }

                for (std::size_t i = movesToApply; i-- > 0;) {
                    const BulkRelocationMove& move = plan.moves[i];
                    if (!moveBulkEntry(move.sourceBucketIndex, scans[i], move.crumb,
                                       move.destinationBucketIndex)) {
                        return false;
                    }
                }
            ...
            // Keep the all-bucket correctness fallback out of the common insertion body. Inlining
            // the serial recursive mover here materially bloats the worker's hot instruction path.
            template<typename BulkTarget, typename BucketLocks>
            __attribute__((noinline, cold)) bool insertOverflowBulkWithAllLocks(
                    const BulkTarget& target, BucketLocks& locks) {
                locks.LockBucketRange(backyard.size());
                const bool inserted = insertOverflowBulk(
                        target.overflow, target.firstBackyardQR,
                        target.secondBackyardQR);
                locks.UnlockBucketRange(backyard.size());
                return inserted;
            }

            // Outlining relocation keeps the overwhelmingly common two-bucket insertion compact.
            template<typename BulkTarget, typename BucketLocks>
            __attribute__((noinline)) bool insertOverflowConcurrentlySlow(
                    const BulkTarget& target, BucketLocks& locks) {
                const std::size_t firstBucketIndex =
                        target.firstBackyardQR.bucketIndex;
                const std::size_t secondBucketIndex =
                        target.secondBackyardQR.bucketIndex;

                // A collection of individually synchronized snapshots can temporarily contain no
                // path even though one exists at a single instant. Bound optimistic retries before
                // taking the definitive, fully locked fallback.
                static constexpr std::size_t ConcurrentRelocationAttempts = 64;
                for (std::size_t attempt = 0;
                     attempt < ConcurrentRelocationAttempts; ++attempt) {
                    std::size_t visited[BulkRelocationDepth];
                    BulkRelocationPlan plan;
                    const bool foundPlan =
                            planBulkRelocation(firstBucketIndex, BulkRelocationDepth,
                                               visited, 0, locks, plan) ||
                            planBulkRelocation(secondBucketIndex, BulkRelocationDepth,
                                               visited, 0, locks, plan);
                    if (!foundPlan) {
                        // Planning is optimistic; retry the synchronized direct case before
                        // rebuilding snapshots instead of treating this as a definitive failure.
                        locks.LockPair(firstBucketIndex, secondBucketIndex);
                        const bool inserted = bulkInsertOverflowNoRelocate(target);
                        locks.UnlockPair(firstBucketIndex, secondBucketIndex);
                        if (inserted) {
                            return true;
                        }
                        _mm_pause();
                    ...
                    std::size_t bucketIndices[2 + 2 * BulkRelocationDepth];
                    std::size_t bucketCount = 0;
                    bucketIndices[bucketCount++] = firstBucketIndex;
                    bucketIndices[bucketCount++] = secondBucketIndex;
                    for (std::size_t i = 0; i < plan.count; ++i) {
                        bucketIndices[bucketCount++] = plan.moves[i].sourceBucketIndex;
                        bucketIndices[bucketCount++] = plan.moves[i].destinationBucketIndex;
                    }
                    std::sort(bucketIndices, bucketIndices + bucketCount);
                    bucketCount = static_cast<std::size_t>(
                            std::unique(bucketIndices, bucketIndices + bucketCount) -
                            bucketIndices);

                    locks.LockBuckets(bucketIndices, bucketCount);
                    const bool inserted = tryBulkRelocationAndInsert(target, plan);
                    locks.UnlockBuckets(bucketIndices, bucketCount);
                    if (inserted) {
                        return true;
                    }
                    _mm_pause();
                ...
                // Repeatedly stale snapshots are indistinguishable from saturation. Locking the
                // complete backyard is a deliberately cold fallback that gives the serial search
                // a definitive view without deferring or dropping this overflow.
                return insertOverflowBulkWithAllLocks(target, locks);
            }
        ...
        public:
            ...
            inline std::size_t getNumBackyardBuckets() const {
                return backyard.size();
            }
            ...
            // Everything an overflow insertion needs: the entry, and the only two
            // backyard buckets that can legally hold it.
            struct BulkOverflowTarget {
                FrontyardQRContainerType overflow;
                BackyardQRContainerType firstBackyardQR;
                BackyardQRContainerType secondBackyardQR;
            };

            inline BulkOverflowTarget bulkOverflowTarget(std::uint64_t packedValue) {
...
#ifdef CUCKOO_HASH
                return BulkOverflowTarget{
                        overflow,
                        BackyardQRContainerType(overflow, 0, R, backyard.size()),
                        BackyardQRContainerType(overflow, 1, R, backyard.size())};
#else
                return BulkOverflowTarget{overflow,
                                          BackyardQRContainerType(overflow, 0, R),
                                          BackyardQRContainerType(overflow, 1, R)};
#endif
            }
            ...
            inline bool bulkInsertOverflowPacked(std::uint64_t packedValue) {
                const BulkOverflowTarget target = bulkOverflowTarget(packedValue);

                lockBackyard(target.firstBackyardQR.bucketIndex,
                             target.secondBackyardQR.bucketIndex);
                const bool inserted = insertOverflowBulk(
                        target.overflow, target.firstBackyardQR, target.secondBackyardQR);
                unlockBackyard(target.firstBackyardQR.bucketIndex,
                               target.secondBackyardQR.bucketIndex);
                return inserted;
            }

            // Bulk-only insertion that never relocates, so it touches nothing but the
            // two buckets in the target and is safe to run concurrently as long as the
            // caller holds those two. Both of them being full is the one case that
            // relocation exists for, and it is left to the caller's relocation protocol.
            inline bool bulkInsertOverflowNoRelocate(const BulkOverflowTarget& target) {
                if (backyard[target.firstBackyardQR.bucketIndex].full() &&
                    backyard[target.secondBackyardQR.bucketIndex].full()) {
                    return false;
                }
            ...
            // Concurrent bulk insertion keeps the common two-choice case to two locks. If both
            // buckets are full, it plans the same depth-limited relocation used by the serial
            // builder, sorts every bucket in that plan, locks the unique set in index order, and
            // revalidates before moving entries. A stale optimistic plan is simply rebuilt.
            template<typename BucketLocks>
            inline bool bulkInsertOverflowConcurrently(
                    std::uint64_t packedValue, BucketLocks& locks) {
                const BulkOverflowTarget target = bulkOverflowTarget(packedValue);
                const std::size_t firstBucketIndex =
                        target.firstBackyardQR.bucketIndex;
                const std::size_t secondBucketIndex =
                ...
                locks.LockPair(firstBucketIndex, secondBucketIndex);
                bool inserted = bulkInsertOverflowNoRelocate(target);
                locks.UnlockPair(firstBucketIndex, secondBucketIndex);
                if (inserted) {
                    return true;
                }
                return insertOverflowConcurrentlySlow(target, locks);
            }
