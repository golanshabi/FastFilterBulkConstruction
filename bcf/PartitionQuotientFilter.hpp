...
namespace PQF {
    ...
    class AlignedVector { //Just here to make locking easier, lol
        private:
            ...
            T* vec;
            bool owned; // False while pointing at memory somebody else allocated.
        ...
        public:
            ...
            // Points the vector at memory it does not own, so a bulk build can turn the
            // buffer that already holds the keys into the buckets themselves.
    ...
    class PartitionQuotientFilter {
        ...
        public:
            ...
            // Puts the front yard in memory the filter does not own. The memory must be aligned
            // to a bucket and hold getNumFrontyardBuckets() of them.
