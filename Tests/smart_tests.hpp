        ...
        void start(std::string filename) {
            if (!started) {
                ...
                if(cpid == 0) {
                    ...
                    sprintf(buf, "perf stat -o %s -p %d --control fd:%d,%d", filename.c_str(), pid, ctl_fd, ack_fd); //,
 ...
 */
    ...
    template<class Table>
    void FPR_test(const std::string &name, std::vector<u64> *v_add, const std::vector<u64> *v_find, std::string path, bool is_hashable, bool create_file = false, bool sorted = false) {
        #ifdef SORT_ONLY
            if (!is_hashable) {
...
uint64_t improved_hash_sort(std::string path, uint64_t vec_size, Table* filter, typename FilterAPI<Table>::StreamBuffer *arr_buf, size_t num_entries) {
    ...
    // PerfManager mgr;
    // uint64_t log_vadd_size = std::log2(num_entries);
    // mgr.start("perf_results_stream/timing_" + std::to_string(log_vadd_size));
    auto start_time = std::chrono::high_resolution_clock::now();
    ...
    while (true) {
        ...
        auto start_hash_time = std::chrono::high_resolution_clock::now();
        FilterAPI<Table>::ImprovedHashVec(&buffer_vec, arr_buf, filter, tmp, num_entries);
        auto end_hash_time = std::chrono::high_resolution_clock::now();
        duration_hash += end_hash_time - start_hash_time;
    ...
    FilterAPI<Table>::FillWithBuffer(arr_buf, filter);
    ...
    template<class Table>
    void FPR_test_with_file(const std::string &name, std::string path, bool is_hashable, size_t num_entries, size_t read_size, size_t max_capacity, bool create_file = false) {
        ...
        if (cpid < 0) {
            perror("fork failed");
            if (arr_buff != nullptr) free(arr_buff - 1);
            exit(1);
        }
        if(cpid == 0) {
            char buf[1024];
        ...
        try {
                ...
                // Verification test: Check that all numbers from the file are found in the filter
                // std::cout << "Running verification test on file..." << std::endl;
                // FileReader reader(path, read_size);
                // std::vector<uint64_t> buffer(read_size);
                // size_t verification_failures = 0;
                // size_t total_checked = 0;
                // size_t total_read = 0;
                // size_t remaining_to_check = num_entries;  // Only check elements that were inserted
            
            // while ((total_read = reader.readNextChunk(buffer, 0)) > 0 && remaining_to_check > 0) {
            //     size_t elements_in_chunk = std::min(total_read, remaining_to_check);
                
            //     for (size_t i = 0; i < elements_in_chunk; i++) {
            //         total_checked++;
            //         if (!FilterAPI<Table>::Contain(buffer[i], wrap_filter)) {
            //             verification_failures++;
            //             std::cout << "VERIFICATION FAILURE: Element " << buffer[i] 
            //                      << " at position " << total_checked << " not found in filter!" << std::endl;
                        
            //             // Stop after first few failures to avoid spam
            //             if (verification_failures >= 10) {
            //                 std::cout << "Too many failures, stopping verification..." << std::endl;
            //                 break;
            //             }
            //         }
            //     }
                
            //     remaining_to_check -= elements_in_chunk;
            //     if (verification_failures >= 10) break;
            // }
            
            // if (verification_failures == 0) {
            //     std::cout << "✓ Verification PASSED: All " << total_checked 
            //              << " elements found in filter" << std::endl;
            // } else {
            //     std::cout << "✗ Verification FAILED: " << verification_failures 
            //              << " out of " << total_checked << " elements not found!" << std::endl;
            // }
