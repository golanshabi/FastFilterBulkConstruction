 ...
 */
    template<class Table>
    size_t get_FPR_test0(Table *wrap_filter, std::vector<u64> *v_add, const std::vector<u64> *v_find, bool sorted, std::vector<u64> *v_check, bool is_hashed) {
        /**Insertion*/
        FilterAPI<Table>::Fill(v_add, wrap_filter, sorted, is_hashed);

        if (std::getenv("TMPCHECK_VERIFY_KEYS") != nullptr) {   // TMPCHECK
            size_t failures = 0;
            for (const uint64_t key : *v_check) {
                failures += !FilterAPI<Table>::Contain(key, wrap_filter);
            }
            std::cout << "[verify] checked=" << v_check->size()
                      << " false_negatives=" << failures << std::endl;
        }
        // size_t counter = 0;
    ...
    template<class Table>
    std::string FPR_test_as_str(Table *wrap_filter, std::vector<u64> *v_add, const std::vector<u64> *v_find, bool sorted, std::vector<u64> *v_check, bool is_hashed) {
        auto yes_res = get_FPR_test0(wrap_filter, v_add, v_find, sorted, v_check, is_hashed);
        // size_t true_counter = yes_res;
        // assert(true_counter <= v_find->size());
        // size_t false_counter = v_find->size() - yes_res;
        // const size_t filter_max_capacity = v_add->size();
    ...
    template<class Table>
    void FPR_test(std::vector<u64> *v_add, const std::vector<u64> *v_find, std::string path, bool is_hashable, bool create_file = false, bool sorted = false, size_t max_capacity = 0) {
        // Filters that are not meant to be filled to their nominal capacity get their size from
        // max_capacity instead of from the number of keys they are given.
        const size_t filter_max_capacity = max_capacity == 0 ? v_add->size() : max_capacity;
        Table filter = FilterAPI<Table>::ConstructFromAddCount(filter_max_capacity);
        Table *wrap_filter = &filter;
        ...
        // if(cpid == 0) {
            ...
            // sprintf(buf, "perf stat -e cycles,cpu-clock,instructions,branch-misses,cache-misses,cache-references -p %d", pid);
