...
inline int SelectRadixBits(size_t num_bits, size_t num_values) {
    // std::cout << "NumBits=" << num_bits << std::endl;
    switch (num_bits) {
        case 14:
            return 6;
        case 15:
 ...
 * @param handle_eviction   (bucket index, evicted 16 bit key) for every key a buffer gave up.
 */
...
inline void HashIntoBucketBuffers(std::vector<u64> *keys, Buffer *buffers, size_t num_buckets,
    ...
    const size_t bits_to_sort = bucket_bits;
        ...
        for (size_t bucket_index = 0; bucket_index < num_buckets; ++bucket_index) {
            ...
            FillSortedBucket<FrontyardLayout>(
                    ...
                    [table](size_t index, const uint16_t *overflowing, size_t count) {
                        for (size_t i = 0; i < count; ++i) {
                            if (!table->bulkInsertOverflowPacked(PackOverflow(index, overflowing[i]))) {
                                throw std::logic_error(
                                        "Breadcrumb Filter backyard insertion failed "
                                        "during bulk construction");
                            }
...
static void radixSortAVX2(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int startingBit = 0) {
...
static void SortPackedVector(std::vector<u64> &v_add,
    ...
    const auto start_time = std::chrono::high_resolution_clock::now();
    const int radix = SelectRadixBits(num_bits, v_add.size());
    radixSortAVX2(v_add, radix, num_bits, temp_arr_to_use);
    const auto end_time = std::chrono::high_resolution_clock::now();
#ifdef TIME_TEST
    const std::chrono::duration<double> elapsed = end_time - start_time;
...
struct FilterAPI<Prefix_Filter<filterTable>> {
     ...
     *
     * A lookup compares the query against the largest key the PD kept and goes to the spare alone
     * when it is bigger, so the PD has to hold the smallest keys of its bucket and keep the
     * largest of them in its last slot. Grouping decides that for every quotient but the one the
     * capacity splits, which SortQuotientAtSplit then sorts.
     */
    static void FillWithBuffer(StreamBuffer *arr_buff, Table *table) {
