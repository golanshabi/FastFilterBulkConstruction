...
static void radixSortBuiltSlowly(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int starting_bit) {
    ...
    for (int shift = starting_bit; shift < totalBits; shift += numBits) {
        ...
        for (size_t i = 0; i < arr.size() - 4; i += 4) {
            uint64_t next_num[4];
            for (int j = 0; j < 4; ++j) {
                uint64_t bucket = (count_num[j] >> shift) & mask;
                next_num[j] = arr[i + 4 + j];
                _mm_prefetch(reinterpret_cast<const char*>(&arr[i + j + 256]), _MM_HINT_T0);
                count[bucket]++;
                count_num[j] = next_num[j];
            }
        }
        for (int j = 0; j < 4; ++j) {
            uint64_t bucket = (count_num[j] >> shift) & mask;
