...
static void SortPackedVector(std::vector<u64> &v_add,
                             ...
                             SortType sort_type, size_t num_bits,
                             double &fill_time_per_entry_ns,
                             bool use_bcf_radix = false, size_t first_bit = 0);
static void radixSortAVX2(std::vector<uint64_t> &arr, int numBits, int totalBits,
                          std::vector<uint64_t> &temp, int firstBit = 0);
...
// Writes as many of one bucket's sorted keys as it can hold and hands the rest over.
...
// A streaming build hashes a key into [bucket index | 16 bit bucket key].
inline constexpr size_t StreamKeyBits = 16;

inline int SelectRadixBits(size_t num_bits, size_t num_values) {
    switch (num_bits) {
        case 14: return 6;
        case 15: return 8;
        case 16: return 6;
...
inline void HashIntoBucketBuffers(std::vector<u64> *keys, Buffer *buffers, size_t num_buckets,
    ...
    const size_t num_bits = StreamKeyBits + bucket_bits;
    const int radix = SelectRadixBits(bucket_bits, keys->size());
    radixSortAVX2(*keys, radix, num_bits, temp, StreamKeyBits);

    for (const u64 packed : *keys) {
        ...
        if (evicted != std::numeric_limits<uint16_t>::max()) {
            handle_eviction(bucket_index, evicted);
        }
    }
}
...
struct FilterAPI<PQF::PartitionQuotientFilter<
    ...
    static_assert(sizeof(StreamBuffer) > FrontyardBucketSize,
                  "A bucket is written over the buffers, so it has to be the smaller of the two");
    ...
    static void ImprovedHashVec(std::vector<u64> *v_orig, StreamBuffer *bucket_array, Table *table,
        ...
        HashIntoBucketBuffers(
                ...
                [table](size_t bucket_index, uint16_t evicted) {
                    if (!table->bulkInsertOverflowPacked(PackOverflow(bucket_index, evicted))) {
        ...
        for (size_t bucket_index = 0; bucket_index < num_buckets; ++bucket_index) {
            ...
            FillSortedBucket<FrontyardLayout>(
                    ...
                    [table](size_t index, const uint16_t *overflowing, size_t count) {
                        for (size_t i = 0; i < count; ++i) {
                            if (!table->bulkInsertOverflowPacked(
                                    PackOverflow(index, overflowing[i]))) {
                                throw std::logic_error(
                                        "Breadcrumb Filter backyard insertion failed "
                                        "during bulk construction");
                            }
    ...
    static void Sort(std::vector<u64> &v_add, Table *table,
                     std::vector<uint64_t> &temp_arr_to_use, SortType sort_type) {
        ...
        SortPackedVector(v_add, temp_arr_to_use, sort_type,
                         FingerprintBits + bucket_bits,
                         fill_time_per_entry_ns, true,
...
static void radixSortAVX2(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int firstBit) {
...
static void radixSortBuiltSlowly(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int firstBit = 0) {
    if (arr.empty() || numBits <= 0 || numBits > 64 || totalBits <= 0 || totalBits > 64) return;
    if (firstBit < 0 || firstBit >= totalBits) return;
    ...
    std::vector<uint32_t> count(bucketCount, 0); // Use vector for bucket counts
    ...
    for (int shift = firstBit; shift < totalBits; shift += numBits) {
        ...
        // Count occurrences of each bucket using AVX2.
        __m256i data = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&arr[0]));
        for (size_t i = 0; i < arr.size() - 4; i += 4) {
            __m256i shifted = _mm256_srli_epi64(data, shift);
            __m256i bucketIndices = _mm256_and_si256(shifted, _mm256_set1_epi64x(mask));

            alignas(32) uint64_t extracted[4];
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(extracted), bucketIndices);

            _mm_prefetch(reinterpret_cast<const char*>(&arr[i + 4 + 256]), _MM_HINT_T0);
            data = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&arr[i + 4]));
            for (int j = 0; j < 4; ++j) {
                count[extracted[j]]++;
            }
        }
...
static void radixSort(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int firstBit = 0) {
    if (arr.empty() || numBits <= 0 || numBits > 64 || totalBits <= 0 || totalBits > 64) return;
    if (firstBit < 0 || firstBit >= totalBits) return;
...
static void SortPackedVector(std::vector<u64> &v_add,
                             ...
                             SortType sort_type, size_t num_bits,
                             double &fill_time_per_entry_ns,
                             bool use_bcf_radix, size_t first_bit) {
    std::cout << "numbers-sorted=" << v_add.size() << std::endl;
    std::cout << "num-bits=" << num_bits << std::endl;
    std::cout << "first-bit=" << first_bit << std::endl;
    const auto start_time = std::chrono::high_resolution_clock::now();
    const int radix = [&]() {
        if (const char *radix_arg = std::getenv("RADIX_BITS")) {
            errno = 0;
            char *end = nullptr;
            const long requested_radix = std::strtol(radix_arg, &end, 10);
            if (errno != 0 || end == radix_arg || *end != '\0' ||
                requested_radix < 1 || requested_radix > 20) {
                throw std::invalid_argument(
                    "radix_bits must be an integer between 1 and 20");
            }
            return static_cast<int>(requested_radix);
        }

        // With the remainder left out of the key the sort covers num_bits - first_bit bits,
        // so the pass count for a given radix shifted by one exponent versus the pre-change
        // code. These thresholds are the measured optima: a wider radix trades a whole pass
        // for a wider scatter, and 1024 buckets is the widest fanout that still pays off, and
        // only up to a few hundred million keys (beyond that the extra pass is cheaper).
        if (use_bcf_radix) {
            const unsigned exponent = std::bit_width(v_add.size() - 1);
            if (exponent == 27 || exponent == 28) return 10;// 28 bits -> 3 passes wins
            if (exponent >= 25 && exponent <= 26) return 9;
            return 8;
        }
        return v_add.size() > (1ULL << 28) ? 9 : 8;
    }();
    std::cout << "radix-bits=" << radix << std::endl;
    switch (sort_type) {
        case SortType::RadixAVX2:
            radixSortAVX2(v_add, radix, num_bits, temp_arr_to_use, first_bit);
            break;
        case SortType::VQSort:
            // Sorts whole keys, so it also orders the bits below first_bit. That is a
            // refinement of what the bulk fill needs, so the extra order is harmless.
            hwy::VQSort(v_add.data(), v_add.size(), hwy::SortAscending());
            break;
        case SortType::Radix:
            radixSort(v_add, radix, num_bits, temp_arr_to_use, first_bit);
            break;
        case SortType::RadixBuiltSlowly:
            radixSortBuiltSlowly(v_add, radix, num_bits, temp_arr_to_use, first_bit);
            break;
        case SortType::IPS2RA: {
            const size_t key_bits = num_bits - first_bit;
            if (key_bits == 0) {
                break;
            }
            // const uint64_t mask = key_bits >= 64 ? ~uint64_t{0}
            //                                      : (uint64_t{1} << key_bits) - uint64_t{1};
            // struct PackedKeyExtractor {
            //     size_t first_bit;
            //     uint64_t mask;
            //     uint64_t operator()(uint64_t v) const {
            //         return (v >> first_bit) & mask;
            //     }
            // };
            ips2ra::sort(v_add.begin(), v_add.end());
                        //  PackedKeyExtractor{first_bit, mask});
            break;
        }
    }
    const auto end_time = std::chrono::high_resolution_clock::now();
...
struct FilterAPI<Prefix_Filter<filterTable>> {
    ...
    static void FillWithBuffer(StreamBuffer *arr_buff, Table *table) {
        ...
        for (size_t pd_index = 0; pd_index < num_pd; ++pd_index) {
    ...
    static void ImprovedHashVec(std::vector<u64> *v_orig, StreamBuffer *bucket_array, Table *table,
        ...
        HashIntoBucketBuffers(
                ...
                [table](size_t pd_index, uint16_t evicted) {
    ...
    static void Sort(std::vector<u64> &v_add, Table *table, std::vector<uint64_t> &temp_arr_to_use, SortType sort_type) {
        ...
        first_bit = 0;
#endif
        SortPackedVector(v_add, temp_arr_to_use, sort_type, num_bits,
                         fill_time_per_entry_ns, false, first_bit);
    }
