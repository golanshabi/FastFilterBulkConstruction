...
static void radixSortBuiltSlowly(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int starting_bit) {
    ...
    for (int shift = starting_bit; shift < totalBits; shift += numBits) {
        ...
        uint64_t count_num = arr[0];
        for (size_t i = 0; i < arr.size() - 1; ++i) {
            _mm_prefetch(reinterpret_cast<const char*>(&arr[i + 1 + 256]), _MM_HINT_T0);
            uint64_t bucket = (count_num >> shift) & mask;
            uint64_t next_num = arr[i + 1];
            count[bucket]++;
            count_num = next_num;
        }
        uint64_t count_bucket = (count_num >> shift) & mask;
        count[count_bucket]++;
        ...
        uint64_t place_num = arr[arr.size() - 1];
        for (int i = arr.size() - 1; i > 0; --i) {
            uint64_t bucket = (place_num >> shift) & mask;
            uint64_t index = --count[bucket];
            _mm_prefetch(reinterpret_cast<const char*>(&temp[index - 4]), _MM_HINT_T0);
            _mm_prefetch(reinterpret_cast<const char*>(&arr[i - 256]), _MM_HINT_T0);
            uint64_t next_num = arr[i - 1];
            temp[index] = place_num;
            place_num = next_num;
        }
        uint64_t place_bucket = (place_num >> shift) & mask;
        uint64_t index = --count[place_bucket];
        temp[index] = place_num;
