...
// Global variable for thread count - can be modified at runtime
...
static std::mutex bloom_filter_lock;
// #define TRACE
#ifndef TRACE
#define DEBUG(msg)
#else
...
// Orders the packed keys by bits [first_bit, num_bits). Leaving the remainder bits out of
// the key saves a whole radix pass, and costs nothing: a bucket only has to be reached with
// its keys grouped by quotient, and the fill sorts the one quotient that needs it (see
// SortLastKeptQuotient).
...
inline void WriteSortedBucket(const u64 *values, size_t count, bool overflowed,
...
// Sorts the keys of the last quotient that fits in the bucket, which is the only place where
// an order by quotient alone is not enough for a layout that records the largest key it kept:
// the keys that stay have to be the smallest ones of their quotient, and the largest of them
// has to be last in the body. The run holds about Capacity/NumQuots keys.
template<typename Layout>
inline void SortLastKeptQuotient(u64 *values, size_t group_size) {
    if (group_size <= Layout::Capacity) {
        return;// A bucket that keeps all of its keys splits no quotient.
    }

    const u64 last_quot = Layout::quot_of(values[Layout::Capacity - 1]);
    size_t begin = Layout::Capacity - 1;
    while (begin > 0 && Layout::quot_of(values[begin - 1]) == last_quot) {
        --begin;
    }
    size_t end = Layout::Capacity;
    while (end < group_size && Layout::quot_of(values[end]) == last_quot) {
        ++end;
    }

    for (size_t i = begin + 1; i < end; ++i) {
        const u64 key = values[i];
        size_t j = i;
        while (j > begin && values[j - 1] > key) {
            values[j] = values[j - 1];
            --j;
        }
        values[j] = key;
...
inline size_t FillSortedBuckets(u64 *packed, size_t num_values,
    ...
    while (begin < num_values) {
        ...
        const bool overflowed = group_size > Layout::Capacity;
        const size_t count = overflowed ? Layout::Capacity : group_size;
        ...
        // The keys arrive ordered by quotient only, so repair the one run that needs a total
        // order here, where the group is still the L1-resident data the scan above walked.
        if constexpr (Layout::TracksOverflow) {
            SortLastKeptQuotient<Layout>(packed + begin, group_size);
        }

        // find start and end of a bucket and then bulk build it
        WriteSortedBucket<Layout>(packed + begin, count, overflowed,
                                  bucket.header, bucket.body);

        if (overflowed) {
            handle_overflow(bucket_index, packed + begin + count, group_size - count);
        }

        entries_in_buckets += count;
        begin = end;
    }
...
// Moves `index` forward to the first key that opens a new bucket, so that a range ending there and
// a range starting there never share a bucket.
template<typename Layout>
inline size_t SnapToBucketStart(const u64 *packed, size_t num_values, size_t index) {
    if (index >= num_values) {
        return num_values;
    }
    if (index == 0) {
    ...
    const size_t previous_bucket = Layout::bucket_of(packed[index - 1]);
    while (index < num_values && Layout::bucket_of(packed[index]) == previous_bucket) {
        ++index;
    }
    return index;
}

// Cuts the sorted keys into one contiguous, bucket-aligned range per thread and lets every thread
// build its own buckets exactly the way a single-threaded build would. Since the ranges never share
// a bucket, the bucket writes need no synchronization.
// One range per thread also separates the threads in the second level, whose bucket index is the
// front-yard bucket index divided by a constant. Cutting the keys finer and handing the ranges out
// dynamically was measured and is slower: the ranges go out in key order, so the running threads
// end up neighbours and contend for the same second-level buckets and lock cache lines.
// `make_handle_overflow(segment)` returns the overflow sink of a single range, which lets callers
// keep the overflows of each thread apart.
template<typename Layout, typename LocateBucket, typename MakeHandleOverflow>
inline size_t FillSortedBucketsParallel(u64 *packed, size_t num_values,
                                        LocateBucket &&locate_bucket,
                                        MakeHandleOverflow &&make_handle_overflow) {
    const int num_segments = std::max(THREAD_NUM, 1);
    const size_t segment_size = num_values / num_segments;

    size_t entries_in_buckets = 0;

    #ifdef _OPENMP
    #pragma omp parallel for num_threads(num_segments) schedule(static) \
            proc_bind(spread) reduction(+ : entries_in_buckets)
    #endif
    for (int segment = 0; segment < num_segments; ++segment) {
        const size_t begin =
                SnapToBucketStart<Layout>(packed, num_values, segment * segment_size);
        const size_t end =
                segment == num_segments - 1
                        ? num_values
                        : SnapToBucketStart<Layout>(packed, num_values,
                                                    (segment + 1) * segment_size);
        if (begin >= end) {
            continue;
        }

        auto handle_overflow = make_handle_overflow(segment);
        entries_in_buckets += FillSortedBuckets<Layout>(
                packed + begin, end - begin, locate_bucket, handle_overflow);
    }
...
struct FilterAPI<PQF::PartitionQuotientFilter<
    ...
    class BackyardLocks {
      public:
        ...
        // Relocation planning takes one bucket at a time. The final commit passes a sorted,
    ...
    // Takes the keys by mutable pointer because the bulk path finishes the sort of a bucket
    // in place, right before it writes that bucket.
    static void Fill(std::vector<u64> *v_add, Table *table, bool sorted, bool is_hashed) {
        ...
        if (!sorted) {
            ...
            #endif
            for (size_t i = 0; i < v_add->size(); ++i) {
                const uint64_t reduced_key = Reduce((*v_add)[i], table);
                if (!table->insert(reduced_key)) {
                    throw std::logic_error(
                        "Breadcrumb Filter backyard insertion failed during construction");
                }
            }
            inserted_entries = v_add->size();
        } else if (!v_add->empty()) {
            ...
            // The backyard takes every key, including relocation cases, while its front-yard range
            // is being built, so no overflow list has to be accumulated and drained afterward.
            const int num_segments = std::max(THREAD_NUM, 1);
            std::atomic<bool> backyard_insert_failed{false};
            BackyardLocks backyard_locks(table->getNumBackyardBuckets());

            start_time = std::chrono::high_resolution_clock::now();
            FillSortedBucketsParallel<FrontyardLayout>(
                    v_add->data(), v_add->size(),
                    [table](size_t bucket_index) {
                        return LocateFrontyardBucket(table, bucket_index);
                    },
                    [table, &backyard_locks, &backyard_insert_failed](int) {
                        return [table, &backyard_locks, &backyard_insert_failed](
                                       size_t, const u64 *overflowing, size_t count) {
                            for (size_t i = 0; i < count; ++i) {
                                if (!InsertOverflowConcurrently(table, backyard_locks,
                                                                overflowing[i])) {
                                    backyard_insert_failed.store(true);
                                }
                            }
                        };
                    });

            if (backyard_insert_failed.load()) {
                throw std::logic_error(
                        "Breadcrumb Filter backyard insertion failed during bulk construction");
            }

            // Every key either fit in its front-yard bucket or reached the backyard.
            inserted_entries = v_add->size();
        }
    ...
    static void HashVec(std::vector<u64> *v_orig, std::vector<u64> *v_hashed, Table *table) {
        ...
        const size_t num_keys = v_orig->size();
        // Indexed through the raw pointers because `at` reloads both vector bounds and adds a
        // compare and a throwing branch to every one of the millions of iterations, to check an
        // index the loop itself produces.
        const u64 *const keys = v_orig->data();
        u64 *const hashed = v_hashed->data();
        #ifdef _OPENMP
        #pragma omp parallel for num_threads(THREAD_NUM) schedule(static)
        #endif
        for (size_t i = 0; i < num_keys; ++i) {
            const uint64_t reduced_key = Reduce(keys[i], table);
            hashed[i] = EncodeBucketFingerprint(reduced_key);
        }
        const auto end_time = std::chrono::high_resolution_clock::now();
#ifdef TIME_TEST
...
class Prefix_Filter {
    ...
    __m256i *pd_array;
    std::mutex *pd_locks;  // Per-PD mutex array for thread safety
    Table GenSpare;
...
public:
    Prefix_Filter(size_t max_items, const float loads[2])
        ...
        // Initialize per-PD mutex array
        #define PD_LOCKS_SIZE 8
        pd_locks = new std::mutex[PD_LOCKS_SIZE];
    ...
    inline void Add(const u64 &item) {
        ...
        std::lock_guard<std::mutex> lock(pd_locks[pd_index % PD_LOCKS_SIZE]);
    ...
    inline void AddHashed(const u64 &s) {
        ...
        // Acquire lock for this specific PD
        std::lock_guard<std::mutex> lock(pd_locks[pd_index % PD_LOCKS_SIZE]);
...
static void doCountParallel(std::vector<uint64_t>& arr, std::vector<uint32_t>& count, std::vector<std::vector<uint32_t>>& per_thread_count, const uint64_t mask, int shift)
{
    int countZero = 0;
    const int simdWidth = 4;
    ...
    for (size_t i = 0; i < arr.size(); i += simdWidth) {
        ...
        for (int lane = 0; lane < simdWidth; ++lane) {
            per_thread_count[thread_id][extracted[lane]]++;
            }
        }
    }
    for (int i = 0; i < per_thread_count[0].size(); i++) {
        count[i] = 0;
        for (int j = 0; j < THREAD_NUM; j++) {
            count[i] += per_thread_count[j][i];
        }
    }
}

static void internalRadixAVX(std::vector<uint64_t>& arr, std::vector<uint64_t>& temp, 
                            std::vector<std::vector<uint32_t>>& count_vec, std::vector<uint32_t>& count,
                            const uint64_t mask, int shift, int bucketCount) {
    ...
    for (int i = 0; i < THREAD_NUM; i++) {
        ...
        auto start_time = std::chrono::high_resolution_clock::now();
    if (THREAD_NUM > 1) {
        doCountParallel(arr, count, count_vec, mask, shift);
    } else {
...
static void radixSortAVX2_internal(std::vector<uint64_t>& arr, std::vector<uint64_t> &temp, int numBits, int totalBits, int startBit) {
    ...
    std::vector<std::vector<uint32_t>> count_vec(THREAD_NUM, std::vector<uint32_t>(bucketCount, 0));
    ...
    for (int shift = startBit; shift < totalBits; shift += numBits) {
        ...
        internalRadixAVX(arr, temp, count_vec, count, mask, actual_shift, bucketCount);
...
// The widest radix a pass over one first-pass range should use.
static constexpr int kMaxPassBits = 11;

// The narrowest radix that sorts `bits` in the fewest passes. Widening a radix pays for itself as
// long as it removes a pass, and once it stops removing one, a narrower radix scatters into fewer
// streams for free.
static constexpr int PassBits(int bits) {
    if (bits <= 0) {
        return kMaxPassBits;
    }
    const int passes = (bits + kMaxPassBits - 1) / kMaxPassBits;
...
static void radixSortAVX2Parallel(std::vector<uint64_t>& arr, int numBits, int totalBits, std::vector<uint64_t> &temp, int firstSortBit, int finalSortBit) {
    ...
    for (int bucket = 0; bucket < numBuckets; ++bucket) {
        ...
        if (end > start) {  // Only sort non-empty buckets
            ...
            bool bucket_result_in_temp = radixSortAVX2(arr, PassBits(totalBits - numBits - finalSortBit),
                                                       totalBits - numBits, temp, finalSortBit, start, end);
...
static void radixSort(std::vector<uint64_t>& arr, int numBits, int totalBits, int firstBit = 0) {
    if (arr.empty() || numBits <= 0 || numBits > 64 || totalBits <= 0 || totalBits > 64) return;
    if (firstBit < 0 || firstBit >= totalBits) return;
    ...
    for (int shift = firstBit; shift < totalBits; shift += numBits) {
        // Reset count array
...
static void SortPackedVector(std::vector<u64> &v_add,
                             ...
                             double &fill_time_per_entry_ns,
                             size_t first_bit) {
    std::cout << "numbers-sorted=" << v_add.size() << std::endl;
    std::cout << "num-bits=" << num_bits << std::endl;
    std::cout << "first-bit=" << first_bit << std::endl;
    const auto start_time = std::chrono::high_resolution_clock::now();
    const int first_pass_bits = firstPassBits();
    radixSortAVX2Parallel(v_add, first_pass_bits, num_bits, temp_arr_to_use,
                          first_pass_bits, first_bit);
    const auto end_time = std::chrono::high_resolution_clock::now();
#ifdef TIME_TEST
    const std::chrono::duration<double> elapsed = end_time - start_time;
...
struct FilterAPI<Prefix_Filter<filterTable>> {
    ...
    static void FillSortedBulk(std::vector<u64> *v_add, Table *table) {
        const size_t entries_in_pds = FillSortedBucketsParallel<PdLayout>(
                v_add->data(), v_add->size(),
                ...
                [table](int) {
                    return [table](size_t pd_index, const u64 *overflowing, size_t count) {
                        for (size_t i = 0; i < count; ++i) {
    ...
    // Takes the keys by mutable pointer because the bulk path finishes the sort of a PD in
    // place, right before it writes that PD.
    static void Fill(std::vector<u64> *v_add, Table *table, bool sorted, bool is_hashed) {
        ...
        if (!sorted) {
            ...
            #else
                // Grouping by PD needs values that were hashed before being sorted; without
                // HASH_AHEAD the sorted order says nothing about the PD index.
                for (auto el : *v_add) {
                    Add(el, table);
                }
            #endif
        ...
        const size_t inserted_entries = v_add->size();
        std::cout << "cap=" << table->get_cap() << " entries_before=" << entries_before << " inserted_entries=" << inserted_entries << std::endl;
        std::cout << "Fill time: " << fill_time_ns << " ns " << " entries: " << inserted_entries << std::endl;
        fill_time_per_entry_ns += compute_fill_time_per_entry_ns(fill_time_ns, inserted_entries);
            ...
            // Save current fill character and set to space
            char old_fill = std::cout.fill(' ');

            // Display elapsed time
            std::cout << std::setw(20) << "Elapsed time:" << " " << elapsed.count() << " s" << std::endl;

            // Display sorted status
            std::cout << std::setw(20) << "Sorted:" << " " << (sorted ? "Yes" : "No") << std::endl;

            // Display size of v_add
            std::cout << std::setw(20) << "v_add.size():" << " " << v_add->size() << std::endl;

            // Display table's capacity
            std::cout << std::setw(20) << "Num in PD:" << " " << table->get_capacity() << std::endl;

            // Display table's cap
            std::cout << std::setw(20) << "Capacity:" << " " << table->get_cap() << std::endl;

            // Display empty PDs count
            std::cout << std::setw(20) << "Num empty PDs:" << " " << table->count_empty_PDs() << std::endl;

            // Display total number of PDs
            std::cout << std::setw(20) << "Total num PDs:" << " " << table->GetNumPd() << std::endl;

            // Restore original fill character
    ...
    static void HashVec(std::vector<u64> *v_orig, std::vector<u64> *v_hashed, Table *table) {
        ...
        // Indexed through the raw pointers because `at` reloads both vector bounds and adds a
        // compare and a throwing branch to every one of the millions of iterations, to check an
        // index the loop itself produces. The bound comes out of the loop condition for the same
        // reason.
        const size_t num_keys = v_orig->size();
        const u64 *const keys = v_orig->data();
        u64 *const hashed = v_hashed->data();
        const uint32_t num_pd = (uint32_t) table->GetNumPd();
        #ifdef _OPENMP
        #pragma omp parallel for num_threads(THREAD_NUM) schedule(static)
        #endif
        for (size_t i = 0; i < num_keys; i++) {
            u64 hashed_val = H0(keys[i]);
            u32 reduced_hash = Table::reduce32(hashed_val >> 32, num_pd);
            uint16_t qr = Table::fixed_reduce(hashed_val);
            u64 saved_val = ((uint64_t) reduced_hash) << 16 | qr;
            hashed[i] = saved_val;
        }
        ...
        std::chrono::duration<double> elapsed = end_time - start_time;
            ...
            std::cout << std::setw(20) << "Hashing Elapsed time:" << std::setw(10) << elapsed.count() << " s" << std::endl;    
        #endif
        uint64_t hashing_time_ns = elapsed_ns(start_time, end_time);
        std::cout << "Hashing time: " << hashing_time_ns << " ns " << " entries: " << v_orig->size() << std::endl;
        fill_time_per_entry_ns += compute_fill_time_per_entry_ns(hashing_time_ns, v_orig->size());\
    }
    ...
    static void Sort(std::vector<u64> &v_add, Table *table, std::vector<uint64_t> &temp_arr_to_use) {
        ...
        size_t first_bit = PdLayout::RemainderBits;
#ifdef TEST_ONLY_BUCKET
        // HashVec drops the fingerprint here, so the low bits are the PD index itself.
        num_bits -= 16;
        first_bit = 0;
#endif
        SortPackedVector(v_add, temp_arr_to_use, num_bits,
                         fill_time_per_entry_ns, first_bit);
    }
