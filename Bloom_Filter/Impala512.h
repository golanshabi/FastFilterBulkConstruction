...
public:
    ...
    void AddUnprotected(const uint64_t key) noexcept;
...
Impala512<HashFamily>::Impala512(const int n)
    ...
    // Allocate mutex array for per-bucket protection
    bucket_mutexes_ = new std::mutex[bucketCount];
...
Impala512<HashFamily>::Add(const uint64_t key) noexcept {
    ...
    // Lock the specific bucket for thread-safe access
    _mm_prefetch(reinterpret_cast<const char*>(&directory_[bucket_idx]), _MM_HINT_T0);
    _mm_prefetch(reinterpret_cast<const char*>(&bucket_mutexes_[bucket_idx]), _MM_HINT_T0);
...
template<typename HashFamily>
[[gnu::always_inline]] inline void
Impala512<HashFamily>::AddUnprotected(const uint64_t key) noexcept {
    const auto hash = hasher_(key);
    const uint32_t bucket_idx = Impala::reduce(Impala::rotl64(hash, 32), bucketCount);
    // const uint32_t bucket_idx = Impala::reduce64(hash, bucketCount);
    const __m512i mask = MakeMask(hash);
    
    // No lock needed for single-threaded or pre-partitioned data
    __m512i *const bucket = &reinterpret_cast<__m512i *>(directory_)[bucket_idx];
    _mm512_store_si512(bucket, _mm512_or_si512(*bucket, mask));
    assert(Find(key));
}
